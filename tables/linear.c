...
struct linear_table {
	...
	int load;
	int flag;	// number of keys in the table right now
	int64 temp;
	int count;
	int prob;
};
...
static void initialise_table(LinearHashTable *table, int size) {
	...
	//table->collisions=malloc((sizeof *table->collisions) * size);
	assert(table->inuse);
	int i;
	for (i = 0; i < size; i++) {
		table->inuse[i] = false;
		//table->collisions[i]=0;
	}
...
static void double_table(LinearHashTable *table) {
	...
	int64 oldkey=table->temp;
	int oldflag=table->flag;
	int oldsize = table->size;
	int oldprob=table->prob;
...
bool linear_hash_table_insert(LinearHashTable *table, int64 key) {
	...
	if(table->inuse[h] && table->slots[h]!=key &&table->flag==1){
	...
	if(linear_hash_table_lookup(table, key) ){
		return false;
	}
	//int oldprob=table->prob;
...
void linear_hash_table_stats(LinearHashTable *table) {
	assert(table != NULL);
	//int count=collisions_count(table);
	printf("--- table stats ---\n");
	printf("number of collisions=%d\n",table->count);
	printf("lenght of average probe sequence =%.4f probes\n",(table->prob+0.0)/(table->load+0.0));
